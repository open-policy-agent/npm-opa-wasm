...
​
    ...
    // Arrays
    "array.concat": (arr1 , arr2) => arr1.concat(arr2),
    "array.slice": (arr, startIndex, stopIndex) => arr.slice(startIndex, stopIndex),
​
    // Strings
    contains: (string, search) => string.includes(search),
    endswith: (string, search) => string.endsWith(search),
    indexof: (string, search) => string.indexOf(search),
    lower: (string) => string.toLowerCase(),
    replace: (string, searchValue, newValue) => string.replace(searchValue, newValue),
    split: (string, delimiter) => string.split(delimiter),
    sprintf: (string, values) => string.sprintf(values),
    startswith: (string, search) => string.startsWith(search),
    substring: (string, start, length) => string.substr(start, length),
    
    //Regex
    re_match: (pattern, value) => RegExp(pattern).test(value),
    "regex.split": (pattern, string) =>  string.split(RegExp(pattern)),
​
    // Types
    is_number: (x) => !isNaN(x),
